...
async function queryWikidata(word: string): Promise<ExternalEvidence | null> {
  ...
  for (const candidate of candidates) {
    ...
    const aliases = (entity.aliases?.ar ?? []).map((alias) => alias.value ?? "").filter(Boolean);
    ...
    const description = entity.descriptions?.ar?.value ?? entity.descriptions?.en?.value ?? candidate.description ?? "";
...
async function queryWikipedia(word: string): Promise<ExternalEvidence | null> {
  ...
  const summaryPayload = (await fetchJson(summaryUrl)) as { title?: string; description?: string | null; extract?: string | null } | null;
